...
const getDiaryForteachers = async (req, res) => {
    const { idKid } = req.params;
    console.log(`[INFO] Solicitud recibida para obtener datos del diario de progenitores para el ID de niño: ${idKid}`);

    try {
        // Consulta a la base de datos para obtener los datos del niño
        console.log('[INFO] Iniciando consulta a la base de datos...');
        const [rows] = await promisePool.query(
            'SELECT * FROM diario_progenitores WHERE id_niño = ? ORDER BY fecha DESC',
            [idKid]
        );
...
module.exports = getDiaryForteachers